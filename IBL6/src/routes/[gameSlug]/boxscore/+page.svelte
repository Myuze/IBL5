<script lang='ts'>
    ...
    const headers = ['Pos', 'Name', ...getAllStatKeys()];
                ...
                <tbody>
                    {#each filteredPlayers as player, rowIndex (player.id || rowIndex)}
                        <tr class="group hover:bg-base-200/50 transition-colors">
                            ...
                            <td class="sticky left-0 z-20 bg-base-100 group-hover:bg-base-200 font-medium px-3 py-1 border-r border-base-300 shadow-lg min-w-40 max-w-40">
                                <div class="sm:truncate text-sm">
                                    {player.name}
                                </div>
                            </td>
                            {#each getAllStatKeys() as statKey}
                                {#if statKey === 'pts'}
